...
<script type="application/javascript" src="/blockchain_votingsystem/js/inputtype.js"></script>
<body>
<div id="headerlog" align="right" style="">
<?php
if ($sign == "admin"){
	$color = "#fff";
	if ($_SERVER['PHP_SELF'] == "/voting_system/FA.php"){
		$logout = "logout";
	}
	else{
		$logout = "../logout";
	...
	if ($logaccount_status == 0){ 
											$color = "#fff";
											$valbut = '<li><a href="" style="background:#fff; color:#ccc">Validate Account</a></li>'; 
										} 
										else { 
											$color = "#f00"; 
											$valbut = '<li data-toggle="modal" data-target="#modal-validate"><a href="" style="color:#f00">Validate Account</a></li>';
    ...
    <td><div class="dropdown">
    			<a href="" class="dropdown-toggle" data-toggle = "dropdown"><i  class="fa fa-gear fa-2x" style="color:<?php echo $color; ?>"></i> </a>
                    <ul class="dropdown-menu nav nav-pills nav-stacked" role="menu" aria-labelledby="dLabel">
                        ...
                        <li style="border-top:1px solid #999"><a href="<?php echo $logout?>.php" onclick="return confirm('You want to logout?');" class="text-success">Log Out</a></li>
                    </ul>
				</div>
    </td>
  </tr>
...
<div class="banner">
    <img src="/blockchain_votingsystem/votingsystem banner.jpg" width="75%" style=""/> 
    </div>
				...
				function verify_validate(validate)
				{
					...
					$.ajax({
						...
						success: function(status){
							...
							if (status == 1)
							{
								$("#validate-notice").html("<p class = 'text-primary' style = 'text-align:center; margin-left:-10%;'>Your account is successfully validated.</p>");
							...
							{
								$("#validate-notice").html("<p class = 'text-danger' style = 'text-align:center; margin-left:-12%;'>Your account is validated already!</p>");
							}							
				...
				function verify_changepass(changepass)
				{
					...
					$.ajax({
						...
						success: function(info){
							...
							{
								$("#changepass-notice").html("<p class = 'text-danger' style = 'text-align:center; margin-left:-11%;'>Password must have atleast of 6 characters!</p>");
							}
